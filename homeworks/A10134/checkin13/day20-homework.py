...
class vehicle():
  def __init__(self,name):
    self.name=name
  def drive(self,distance,day,hour):
    speed=distance/hour
    print '%s 骑 %s 平均时速 %0.2f km/h' %(day,self.name, speed)

def main():
  e_bicycle=vehicle('电动车')
  bicycle=vehicle('自行车')
  distance=20
  e_bicycle.drive(distance,"周一",0.5)
  bicycle.drive(distance,'周二',2)
  e_bicycle.drive(distance,"周三",0.6)
  # distance = 20
