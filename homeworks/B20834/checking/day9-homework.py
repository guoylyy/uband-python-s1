...
def homework1():
	...
	dictionary['bad'] = 'failing to reach an acceptable level'
	del dictionary['bad']
	print dictionary
...
def homework2():
	...
	if dictionary.has_key('etiquette'):
		print 'etiquette的解释是:', dictionary['etiquette']
	else:
		del dictionary['abandon']
		print '老爸先查了一个单词etiquette没有查到'
		print '老爸怒了，把含有abandon 一页的单词撕掉了'
		print dictionary
	if dictionary.has_key('abase'):
		print '然后老爸又查了一个单词abase得到了解释'
		for key in dictionary:
			if key == 'abase':
				print 'abase的解释是:', dictionary['abase']
		print '老爸很开心，又把abandon 加入到了字典里'
		dictionary['abandon'] = 'to give up to the control or influence of another person or agent'
		print dictionary
		print '----------------'
