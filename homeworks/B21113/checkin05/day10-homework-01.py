...
def homework2():
  ...
  if not book.has_key('etiquette'): #没有查到
    ...
    #查询
  print '老妈又查了abase,abash两个词'	
  if book.has_key('abase'):
  		print '查到了%s' %('abase:to lower in rank, office, prestige, or esteem')
  else:
  		print '没有abase这个词'

  if book.has_key('abash'):
  		print '查到了%s ' %('abashto: destroy the self-possession or self-confidence of')
  else:
  	print '没有abash这个单词'
