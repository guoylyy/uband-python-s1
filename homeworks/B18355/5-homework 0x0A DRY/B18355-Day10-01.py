...
# 做几件事情
# 1. 把老爸换成老妈，做的时候数一数改了几处代码______7
# 2. 把撕毁的页码换成'abase'，做的时候数一数改了几处代码_____5
# 3. 想想如何让最后粘贴回 撕毁代码页 的时候不用重复再打一遍长串释意
#     ---把撕下来的一页也存为一个字典形式，然后贴回去的时候，直接是字典元素等于字典元素
# 
#4. 如果查到就打印:'老爸查到了 %s : %s', 写代码让老爸再搜索'abase' 'abash'这两个单词
#    数一数有多少处重复的打印代码______3
# 
...
def homework2():
  ...
  if not book.has_key('etiquette'): #没有查到
    print ' Mama没有查到 %s 的意思' % ('etiquette')

    #撕毁了abandon
    del book['abase']
    print ' Mama撕毁了 %s 的页面' % ('abase')

    if book.has_key('abase'):
      print ' Mama查到了 %s : %s' % ('abase', book['abase'])

      #老爸黏贴了代码
      book['abase'] = 'to lower in rank, office, prestige, or esteem'
      print ' Mama把 %s 的字典页又贴上了' % ('abase')

    else:
      print ' Mama没有查到 %s ' % ('abase') 
  else:
    print ' Mama查到了 %s '% ('etiquette')
  ...
  if book.has_key('abash'):
    print ' Mama查到了 %s : %s' % ('abash', book['abash'])
  else:
    print ' Mama没有查到 %s ' % ('abash') 
