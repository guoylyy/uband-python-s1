...
# 做几件事情
# 1. 把老爸换成老妈，做的时候数一数改了几处代码           8(加上注释)
# 2. 把撕毁的页码换成'abase'，做的时候数一数改了几处代码  4
# 
# 3. 想想如何让最后粘贴回 撕毁代码页 的时候不用重复再打一遍长串释意
# 
# 4. 如果查到就打印:'老爸查到了 %s : %s', 写代码让老爸再搜索'abase' 'abash'这两个单词
#    数一数有多少处重复的打印代码                         很多重复，很烦人=,=
# 
...
def homework2():
  ...
  if not book.has_key('etiquette'): #没有查到
    print 'mom没有查到 %s 的意思' % ('etiquette')

    #撕毁了abase 
    del book['abase']
    print 'mom撕毁了 %s 的页面' % ('abase')

    if book.has_key('abase'):
      print 'mom查到了 %s : %s' % ('abase', book['abase'])

      #mom黏贴了代码
      book['abandon'] = 'to give up to the control or influence of another person or agent'
      print 'mom把 %s 的字典页又贴上了' % ('abase')

    else:
      print 'mom没有查到 %s ' % ('abase') 

    if book.has_key('abash'):
        print 'mom查到了 %s : %s' % ('abash', book['abash'])
    else:
        print 'mom没有查到 %s ' % ('abash') 

  else:
    print 'mom查到了 %s '% ('etiquette')
