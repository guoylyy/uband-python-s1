...
def homework2():
  ...
  print '老妈在看一本英文书 '#题1.老爸改成老妈共7处
  if not book.has_key('etiquette'): #没有查到
    print '老妈没有查到 %s 的意思 ' % ('etiquette')

    #撕毁了abandon
    del book['abase']#题2.撕毁的页面变成abase改了4处
    print '老妈撕毁了 %s 的页面 ' % ('abase')

    if book.has_key('abase'):
      print '老妈查到了 %s : %s ' % ('abase', book['abase'])
      #老爸黏贴了代码
      book['abase'] = 'to give up to the control or influence of another person or agent'#题3.可以定义一个字符串来代替
