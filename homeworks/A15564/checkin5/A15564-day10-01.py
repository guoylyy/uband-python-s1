...
"提前将释义储存在一个变量中，粘贴回的时候直接调用该变量，如  meaning = book['abase']  "
...
def homework2():
  ...
  if not book.has_key('etiquette'): #没有查到
    ...
    if book.has_key('abase'):
      ...
      print '老妈没有查到 %s ' % ('abase') ## 原代码中abase处没引号
    
    ##贴回了abandon → abase     abandon释义改为abase释义
    book['abase'] = 'to lower in rank, office, prestige, or esteem'
    print '老妈把 %s 的字典页又贴上了' % ('abase')   ## 贴回字典应在if语句外
  else:
